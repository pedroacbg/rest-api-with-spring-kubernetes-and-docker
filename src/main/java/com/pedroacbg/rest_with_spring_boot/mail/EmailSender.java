package com.pedroacbg.rest_with_spring_boot.mail;
...
public class EmailSender implements Serializable {
    ...
    public void send(EmailConfig config){
        ...
        try {
            ...
            helper.setText(body, true); // o true é para indicar que é html
            if (attachment != null){
                helper.addAttachment(attachment.getName(), attachment);
            }
            javaMailSender.send(message);
            logger.info("Email sent to %s with the subject '%s'%n", to, subject);
            reset(); // metodo para resetar as configurações para evitar de enviar as mesmas informações
    ...
    // pega os emails e splita em varios internet address e cria uma lista
